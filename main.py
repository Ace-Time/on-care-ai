...
대상 독자는 관리자이며, 관리자는 이 요약을 보고 해당 월의 수급자 상태 경향을 빠르게 파악해야 해.
...
- "특이점" 기준은 월 전체 빈도로 판단하고, 아래 기준 중 하나라도 만족하면 1문장으로 포함:
  (1) 해당 월 전체에서 1회만 등장한 항목
  (2) special_note에 통증/부종/피부문제/수면부족/배뇨실수 등 주의 신호가 포함
  (3) 상태(신체/기분/수면)에서 평소와 다른 항목이 단발성으로 등장
- 자주 수행된 신체활동(식사/배설/위생/이동) 경향을 반영
- 인지/정서 지원 경향을 반영
- 상태(신체/기분/수면)의 주의점을 반영 (단정 금지: “관찰됨/주의 필요” 톤)
- 반복되는 특이사항이 있으면 강조
